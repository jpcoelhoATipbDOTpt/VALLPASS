...
//        cout << "outputs: " << interpreter->outputs().size() << "\n";
    ...
    //there are ALWAYS 10 detections no matter how many objects are detectable
        ...
        //cout << "classe: " << detection_classes[0] << "\n";
    ...
    for(int i = 0; i < num_detections; i++){
        if(detection_scores[i] > confidence_threshold){
            ...
            if (strcmp(Labels[det_index].c_str(), "person") == 0)
                {
                putText(src, "Person Present", cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2);
                ...
                cv::Mat frameDiff;
                cv::absdiff(prevFrame, src, frameDiff);
                cv::cvtColor(frameDiff, frameDiff, cv::COLOR_BGR2GRAY);
                cv::threshold(frameDiff, frameDiff, 30, 255, cv::THRESH_BINARY);

                int motionPixels = cv::countNonZero(frameDiff);
                int motionThresholdPixels = 2500;  // Adjust this threshold based on your requirements
                cout << "classe: " << motionPixels << "\n";
                if (motionPixels > motionThresholdPixels)
                {
                    // Motion detected
                    cv::putText(src, "Motion Detected", cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2);
                }
